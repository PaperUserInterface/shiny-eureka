...
async function query(sql, params = []) {
  await dbReady; // Wait until DB tables and seeding are complete
...
async function seedInitialData() {
  try {
    // 1. Seed Food Items
    const foodCheck = await queryDirect('SELECT COUNT(*) as count FROM food');
    const foodCount = parseInt(foodCheck.rows[0].count || foodCheck.rows[0]['COUNT(*)'] || 0, 10);

    if (foodCount === 0) {
      console.log('🍔 [Database] Seeding food items...');
      const defaultFoods = [
        ['Classic Burger', 8.99],
        ['Cheeseburger', 9.99],
        ['Bacon Deluxe Burger', 11.50],
      ...
      for (const [name, price] of defaultFoods) {
        await queryDirect('INSERT INTO food (name, price) VALUES ($1, $2)', [name, price]);
      }
      console.log('✅ [Database] 18 Food items seeded.');
    }

    // 2. Seed Accounts if missing
    const userCheck = await queryDirect('SELECT COUNT(*) as count FROM users');
...
// Internal query function used before dbReady is resolved
    ...
    return new Promise((resolve, reject) => {
      ...
      if (trimmed.startsWith('SELECT') || trimmed.includes('RETURNING')) {
        sqliteDb.all(sqliteSql, params, (err, rows) => {
          if (err) return reject(err);
          const sanitizedRows = (rows || []).map(row => {
            if ('banned' in row) row.banned = Boolean(row.banned);
            return row;
          });
          resolve({ rows: sanitizedRows, rowCount: sanitizedRows.length });
        });
