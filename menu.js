...
document.addEventListener('DOMContentLoaded', async () => {
  await checkAuth(); // Optional check
  await loadMenu();
});
...
async function loadMenu() {
  ...
  try {
    ...
    if (!res.ok) {
      grid.innerHTML = '<div class="alert alert-danger">Failed to load menu items from server.</div>';
      return;
    }
    ...
    if (!Array.isArray(foodItems) || foodItems.length === 0) {
      grid.innerHTML = '<p style="color: var(--text-muted);">No food items available right now.</p>';
      return;
    }
    ...
    grid.innerHTML = '<div class="alert alert-danger">Failed to load menu items. Please refresh the page.</div>';
  }
}
  ...
  if (!currentUser) {
    alertBox.innerHTML = `
      <div class="alert alert-danger">
        You must be logged in to place an order. <a href="/login.html" style="font-weight: 700;">Log In Here</a>
      </div>
    `;
    return;
  }
